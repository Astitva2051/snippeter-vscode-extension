...
var SECRET_KEY = "snippet-er.token";
async function storeToken(token) {
  await vscode.workspace.getConfiguration().update(SECRET_KEY, token, vscode.ConfigurationTarget.Global);
}
async function getStoredToken() {
  return vscode.workspace.getConfiguration().get(SECRET_KEY) || null;
}
async function clearToken() {
  await vscode.workspace.getConfiguration().update(SECRET_KEY, null, vscode.ConfigurationTarget.Global);
}
...
async function loginUser() {
  try {
    const email = await vscode2.window.showInputBox({
      ...
      validateInput: (value) => !validateEmail(value) ? "Please enter a valid email address" : null
    });
    if (!email) {
      return;
    }
    const password = await vscode2.window.showInputBox({
      prompt: "Enter your password",
      password: true,
      // Mask input
      validateInput: (value) => value.length < 6 ? "Password must be at least 6 characters" : null
    });
    if (!password) {
      return;
    }
      ...
      if (error.response?.status === 401) {
        vscode2.window.showErrorMessage(
          ...
          if (selection === "Register") {
            vscode2.env.openExternal(vscode2.Uri.parse(FRONTEND_REGISTER_URL));
          }
        });
      } else {
        vscode2.window.showErrorMessage(
...
async function logoutUser() {
  try {
    await clearToken();
    vscode2.window.showInformationMessage("Logged out successfully!");
  } catch (error) {
    vscode2.window.showErrorMessage(`Logout failed: ${error}`);
  }
}
      ...
      if (axiosError.response) {
        switch (axiosError.response.status) {
          ...
          case 403:
            ...
            break;
          case 404:
            vscode3.window.showErrorMessage("Snippets endpoint not found.");
            break;
          case 500:
            vscode3.window.showErrorMessage(
...
async function createSnippet(snippetData) {
  try {
    ...
    if (!snippetData.title || !snippetData.code || !snippetData.language) {
      vscode3.window.showErrorMessage("Incomplete snippet details.");
      return false;
    }
    const response = await axios_default.post(
...
function throttle2(func, delay = 300, options = { leading: true, trailing: true }) {
  let timeoutId = null;
  let lastExecution = 0;
  let lastArgs = null;
  let lastThis = null;
...
var SupportedLanguage = /* @__PURE__ */ ((SupportedLanguage2) => {
  ...
  SupportedLanguage2["Java"] = "java";
  SupportedLanguage2["C"] = "c";
  SupportedLanguage2["CPP"] = "cpp";
  SupportedLanguage2["CSharp"] = "csharp";
  SupportedLanguage2["Go"] = "go";
  SupportedLanguage2["Rust"] = "rust";
  SupportedLanguage2["Ruby"] = "ruby";
  SupportedLanguage2["PHP"] = "php";
  SupportedLanguage2["Swift"] = "swift";
  SupportedLanguage2["Kotlin"] = "kotlin";
  SupportedLanguage2["SQL"] = "sql";
  SupportedLanguage2["HTML"] = "html";
  SupportedLanguage2["CSS"] = "css";
...
async function showLanguageQuickPick() {
  ...
  return await vscode5.window.showQuickPick(languages2, {
...
async function saveSnippet() {
  try {
    const token = await getStoredToken();
    if (!token) {
      const loginResponse = await vscode5.window.showErrorMessage(
    ...
    const textEditor = vscode5.window.activeTextEditor;
    if (!textEditor) {
    ...
    const tagsInput = await vscode5.window.showInputBox({
    ...
    vscode5.window.showErrorMessage(`Failed to save snippet: ${error}`);
  }
}
function activate(context) {
  ...
  context.subscriptions.push(
    vscode5.commands.registerCommand("extension.saveSnippet", saveSnippet)
  );
  const snippetStatusBarItem = vscode5.window.createStatusBarItem(
  ...
  async function handleLoggedInClick() {
    ...
    if (action === "Create Snippet") {
      vscode5.commands.executeCommand("extension.saveSnippet");
    ...
    context.subscriptions.push(
      vscode5.languages.registerCompletionItemProvider(
        language,
        {
          ...
          provideCompletionItems: throttle2(async (document2, position) => {
            const textLine = document2.lineAt(position.line).text.substring(0, position.character);
            ...
            const matchingSnippets = snippets.filter((snippet) => {
              ...
              const tagScore = snippet.tags ? Math.max(
                ...snippet.tags.map(
                  (tag) => calculateMatchScore(tag, typedText)
                )
              ) : 0;
              return titleScore > 0.5 || tagScore > 0.5;
            });
            return matchingSnippets.map((snippet) => {
              const completionItem = new vscode5.CompletionItem(
                snippet.title,
                vscode5.CompletionItemKind.Snippet
              );
              completionItem.insertText = new vscode5.SnippetString(
                snippet.code
              );
              completionItem.documentation = new vscode5.MarkdownString(
                `**Time Complexity:** ${snippet.timeComplexity || "N/A"}

**Space Complexity:** ${snippet.spaceComplexity || "N/A"}
...
\`\`\``
  ...
  vscode5.window.showInformationMessage("Snippet-er Extension Activated!");
}
function deactivate() {
